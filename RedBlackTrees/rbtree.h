/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Hunter Bartholomew and Ankush Dave
 * Version     : 1.0
 * Date        : 11/22/2016
...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        RedBlackNode<K, V> *inserted = new RedBlackNode<K, V>(key, value);
        if (root_ != NULL && check_duplicates(root_, inserted)) {
        ...
        if (y == NULL) { root_ = inserted; }
        else if (inserted->key() < y->key()) { y->set_left(inserted); }
        else { y->set_right(inserted);}
        inserted->set_parent(y);
        size_++;
    ...
    bool check_duplicates(Node<K, V> *root, Node<K, V> *search) {
        ...
        if (search->value() == root->value())
        	return true;
        return (check_duplicates(root->left(), search) || check_duplicates(root->right(), search));
    }
